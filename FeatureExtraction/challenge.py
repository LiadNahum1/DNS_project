...
def build_train_set(user_id, all_features_of_all_users):
    ...
    for other_user in range(0, USER_COUNT):
        if other_user != user_id:
            train_set.extend(all_features_of_all_users[other_user][
                             :segments_of_other_users])  # taking two segemnts from each other user
...
def calculate_grade():
    ...
    for i in range(0, 10):
        ...
        print(
            f'true_positive: {check_true_positive.count(True)} true_negative: {check_true_negative.count(True)} false positive: {false_positive}')
